...
import dmx.lighting.UpdateDmxBlockDisplayPayload;
...
/** Console editor for one summonable DMX Block Display. */
public final class DmxBlockDisplayScreen extends Screen {

    private static final int FIELD_HEIGHT = 20;
    private static final int ROW_SPACING = 26;
    private static final int CHANNEL_FIELD_WIDTH = 64;
    ...
    private int selectedSkin = DmxBlockDisplayEntity.MIN_SKIN;
    private boolean colorInterpolationEnabled;
    private float colorInterpolationTimeSeconds =
            DmxFixtureBlockEntity.DEFAULT_COLOR_INTERPOLATION_TIME_SECONDS;
    private Component statusMessage = Component.empty();
    private int statusColor = 0xFFFFFFFF;

    public DmxBlockDisplayScreen(FixtureBrowserEntry entry) {
        super(Component.literal("DMX Block Display"));
        this.entry = entry;
    }

    @Override
    protected void init() {
        selectedSkin = entry == null
                ? DmxBlockDisplayEntity.MIN_SKIN
                : entry.skin();
        colorInterpolationEnabled = entry != null
                && entry.colorInterpolationEnabled();
        colorInterpolationTimeSeconds = entry == null
        ...
        nameField = createTextField(
                ...
                entry == null ? "" : entry.fixtureName(),
                "DMX Block Display name"
        );
        universeField = createTextField(
                ...
                80,
                3,
                entry == null ? "1" : Integer.toString(entry.universe()),
                "DMX universe"
        );
        groupField = createTextField(
                ...
                entry == null || entry.isUngrouped()
                        ? ""
                        : entry.groupName(),
                "DMX group"
        );

        int channelTop = top + ROW_SPACING * 3 + 22;
        redChannelField = createChannelField(
                fieldX,
                channelTop,
                entry == null
        ...
        greenChannelField = createChannelField(
                fieldX,
        ...
        blueChannelField = createChannelField(
                fieldX,
        ...
        strobeChannelField = createChannelField(
                fieldX,
                channelTop + ROW_SPACING * 4,
                entry == null
                        ? FixtureParameterMap.UNASSIGNED
                        : entry.strobeChannel(),
                "Strobe DMX channel"
        ...
        skinButton = addRenderableWidget(
                ...
                .bounds(
                        fieldX,
                        channelTop + ROW_SPACING * 5,
                        100,
                        FIELD_HEIGHT
                )
                .build()
        );
        skinDmxChannelField = createChannelField(
                fieldX + 110,
                channelTop + ROW_SPACING * 5,
                entry == null
                        ? FixtureParameterMap.UNASSIGNED
        ...
        int fadeY = channelTop + ROW_SPACING * 6 + 6;
        colorInterpolationButton = addRenderableWidget(
                Button.builder(
                        fadeLabel(),
                        button -> toggleColorInterpolation()
                )
                .bounds(fieldX, fadeY, 105, FIELD_HEIGHT)
                .build()
        );
        colorInterpolationTimeField = createTextField(
                fieldX + 115,
                fadeY,
                72,
                6,
                formatInterpolationSeconds(colorInterpolationTimeSeconds),
                "Color fade time in seconds"
        );

        int buttonY = fadeY + ROW_SPACING + 8;
        addRenderableWidget(
                Button.builder(
                        Component.literal("Save"),
                        button -> saveConfiguration()
                )
                .bounds(fieldX, buttonY, 105, FIELD_HEIGHT)
                .build()
        );
        addRenderableWidget(
                Button.builder(
                ...
                .bounds(fieldX + 115, buttonY, 105, FIELD_HEIGHT)
                .build()
        );
    ...
    private boolean saveConfiguration() {
        if (entry == null || !entry.isMobTarget()) {
            setError("Select a loaded DMX Block Display first.");
            return false;
        }
        ...
        Integer redChannel = parseChannel(redChannelField);
        Integer greenChannel = parseChannel(greenChannelField);
        Integer blueChannel = parseChannel(blueChannelField);
        Integer dimmerChannel = parseChannel(dimmerChannelField);
        Integer strobeChannel = parseChannel(strobeChannelField);
        Integer skinDmxChannel = parseChannel(skinDmxChannelField);
        ...
        if (!validateColorInterpolationTime()) {
            setError("Color fade time must be between 0.05 and 60 seconds.");
            return false;
        }
        ...
        if (!ClientPlayNetworking.canSend(
            ...
            setError("DMX Block Display packet is unavailable.");
            return false;
        }

        ClientPlayNetworking.send(
                new UpdateDmxBlockDisplayPayload(
        ...
        statusMessage = Component.literal("DMX Block Display saved.");
    ...
    private void selectNextSkin() {
        selectedSkin++;
        if (selectedSkin > DmxBlockDisplayEntity.MAX_SKIN) {
            selectedSkin = DmxBlockDisplayEntity.MIN_SKIN;
        }
        skinButton.setMessage(skinLabel());
    }

    private Component skinLabel() {
        return Component.literal("Skin: " + selectedSkin);
    }

    private void toggleColorInterpolation() {
        boolean previous = colorInterpolationEnabled;
        colorInterpolationEnabled = !colorInterpolationEnabled;
        colorInterpolationButton.setMessage(fadeLabel());

        if (!saveConfiguration()) {
            colorInterpolationEnabled = previous;
            colorInterpolationButton.setMessage(fadeLabel());
            return;
        }

        statusMessage = Component.literal(
                colorInterpolationEnabled
                        ? "Color fade enabled."
                        : "Color fade disabled."
        );
    ...
    private Component fadeLabel() {
        return Component.literal(
                colorInterpolationEnabled ? "Fade: On" : "Fade: Off"
        );
    }

    private boolean validateColorInterpolationTime() {
        Double parsed = parseDouble(colorInterpolationTimeField.getValue());
        ...
        colorInterpolationTimeField.setTextColor(
                valid ? 0xFFFFFFFF : 0xFFFF5555
        );
        if (valid) {
            colorInterpolationTimeSeconds = parsed.floatValue();
        }
        return valid;
    }
    ...
    private static String formatInterpolationSeconds(float seconds) {
    ...
    public void extractRenderState(
        ...
        graphics.text(font, "Name", labelX, top + 6, 0xFFFFFFFF, false);
        graphics.text(
                font,
                "Universe",
                labelX,
